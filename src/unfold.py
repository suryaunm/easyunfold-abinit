...
from .wavefun import VaspWaveFunction, CastepWaveFunction, AbinitWaveFunction
...
class UnfoldKSet(MSONable):
    ...
    def __init__(self,
        ...
        :param dft_code: Name of the DFT code to be used. Supported: ``'vasp'``, ``'castep'``, ``'abinit'``.
        """
...
class Unfold:
    ...
    def __init__(self,
        ...
        self.time_reversal = time_reversal
        self.dft_code = dft_code          # ADD THIS LINE
        ...
        elif dft_code == 'abinit':
            # gamma and gamma_half are VASP-only flags; lsorbit is shared and
            # auto-detected from nspinor inside AbinitWaveFunction.from_file.
            self.wfc = AbinitWaveFunction.from_file(fname, lsorbit=self._lsoc)
        else:
            raise NotImplementedError(f"DFT code '{dft_code}' has not been implemented! "
                                      f"Supported codes: 'vasp', 'castep', 'abinit'.")
    ...
    def get_vbm_cbm(self, thresh: float = 1e-8) -> Tuple[float, float]:
        """Locate the VBM and CBM from the wave function data.
        For Abinit band structure runs all occupancies are zero,
        so fall back to the Fermi energy stored in the WFK file."""
        occ = self.wfc.occupancies
        occupied = np.abs(occ) > thresh

        if not occupied.any() and hasattr(self.wfc, 'fermie') and self.wfc.fermie is not None:
            # Non-SCF run: use Fermi energy from WFK as both VBM and CBM
            fermie = self.wfc.fermie
            return fermie, fermie

        if not occupied.any():
            # No occupancy info and no Fermi energy - return band midpoint
            nb = self.bands.shape[-1]
            vbm = float(self.bands[:, :, :nb // 2].max())
            cbm = float(self.bands[:, :, nb // 2:].min())
            return vbm, cbm
    ...
    def spectral_weight_k(self, k0, whichspin=1):
        ...
        # mesh_size is required to map G-vectors onto a 3-D FFT grid.
        # For Abinit it comes from number_of_grid_points_vector1/2/3; if those
        # dimensions were absent in the file we fall back to a grid large enough
        # to hold every G-vector actually present in the data.
        mesh_size = self.wfc.mesh_size
        if mesh_size is None:
            mesh_size = np.max(np.abs(Gall), axis=0) * 2 + 1

        # Index of the Gvalid in 3D grid
        GallIndex = Gall % mesh_size[np.newaxis, :]
        ...
        GoffsetIndex = Goffset % mesh_size[np.newaxis, :]

        # 3d grid for planewave coefficients
        wfc_k_3D = np.zeros(mesh_size, dtype=np.complex128)
