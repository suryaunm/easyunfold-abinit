...
SUPPORTED_DFT_CODES = ('vasp', 'castep', 'abinit')
...
def generate(pc_file, code, sc_file, matrix, kpoints, time_reversal, out_file, no_expand, symprec, nk_per_split, scf_kpoints, yes,
    ...
    if code == 'vasp':
        ...
        # Abinit kpoints are written in VASP KPOINTS format (see utils.py);
        # use a recognisable name so users know what to look for.
        out_kpt_name = f'KPOINTS_{out_file.stem}'
    else:
...
def unfold_calculate(ctx, wavefunc, save_as, gamma, ncl):
    ...
    Multiple wave function (e.g. the WAVECAR file if using VASP, or the WFK.nc file if using
    Abinit) files can be supplied for split-path calculations.
    ...
    Note: ``--gamma`` and ``--ncl`` are VASP-only flags and are silently ignored for
    Abinit and CASTEP calculations.
    """
    from easyunfold.unfold import UnfoldKSet
