...
def write_kpoints(kpoints: Union[np.ndarray, list], outpath, *args, code='vasp', **kwargs):
    ...
    :param code: The code that the kpoint file is used for.
                 Supported values: ``'vasp'``, ``'castep'``, ``'abinit'``.
                 For ``'abinit'``, kpoints are written in VASP KPOINTS format.
    """
    ...
    if code == 'abinit':
        # Abinit kpoints are supplied directly in the input file; we write
        # them in VASP KPOINTS format so they can be inspected / converted.
        return write_kpoints_vasp(kpoints, outpath, *args, **kwargs)
...
def read_kpoints(path='KPOINTS', code='vasp', **kwargs):
    ...
    Supported values: ``'vasp'``, ``'castep'``, ``'abinit'``.
    For ``'abinit'``, the file is expected to be in VASP KPOINTS format.
    """
    if code == 'vasp':
    ...
    if code == 'abinit':
        # Abinit kpoints are written in VASP format by write_kpoints; read
