...
LABELS = [
    r"Mo$_{0.25}$Nb$_{0.75}$_old",
    #"Mo",
    #"Nb",
    #r"Mo$_{0.25}$Nb$_{0.75}$_sqs4",
    #r"Mo$_{0.25}$Nb$_{0.75}$_sqs11",
    #"W",
    # "Run 3",
]
...
WEIGHT_THRESHOLD = 0.5   # ONLY plot eigenvalues with spectral weight > this value
...
INTENSITY  = 10.0          # higher = brighter bands  (try 3–10)
...
def compute_spectral_function(data, npoints=2000, sigma=0.1, weight_threshold=0.0):
    """
    ...
    weight_threshold : float
        Only eigenvalues with spectral weights greater than this value are kept.
    ...
    for ispin, ik in itertools.product(range(nspin), range(nk)):
        ...
        for jsub in range(n_sub):
            ...
            # FILTERING: keep only states with weight > weight_threshold
            mask = P_Km > weight_threshold
            E_Km = E_Km[mask]
            P_Km = P_Km[mask]
            ...
            # Lorentzian smearing: shape (npoints, filtered_nbands) -> sum over bands
            contrib = _lorentzian(
                eng[:, np.newaxis],    # (npoints, 1)
                E_Km[np.newaxis, :],   # (1, filtered_nbands)
                sigma,
            )
...
def plot_multi_unfold(
    ...
    sigma : float
        Lorentzian broadening width in eV.
    weight_threshold : float
        Minimum spectral weight required to include an eigenvalue in the plot.
    figsize : (float, float)
    ...
    for i, (path, label, color, this_eref) in enumerate(
            zip(json_files, labels, colors, erefs)):
        ...
        eng, sf = compute_spectral_function(
            data, 
            npoints=npoints, 
            sigma=sigma, 
    ...
    # fig.savefig("Mo25Nb_sqs_comp.png", dpi=dpi, bbox_inches="tight") # Optional default save
    fig.savefig("Mo25Nb_old_0p5sw.png", dpi=dpi, bbox_inches="tight")
